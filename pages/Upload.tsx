...
export const Upload: React.FC<UploadProps> = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
            {/* Sub-tabs */}
            <div className="bg-white dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 transition-colors">
                    ...
                    <div className="flex gap-1">
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-4 font-semibold transition-all ${activeSubTab === 'csv'
                                ? 'text-brand-600 dark:text-brand-400 border-b-2 border-brand-600 dark:border-brand-400 bg-brand-50/30 dark:bg-brand-900/20'
            ...
            {activeSubTab === 'import' ? (
                ...
                <div className="p-6">
                    <div className="max-w-6xl mx-auto">
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Upload de Planilha</h1>
                        <p className="text-slate-600 dark:text-slate-400 mb-8">Envie uma planilha CSV com os dados de produtos para atualizar o banco de dados</p>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 mb-8 transition-colors">
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Selecionar Arquivo</h2>
                            ...
                            {!file ? (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-4">Formato aceito: CSV</p>
                                </div>
                            ) : (
                                <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-6 bg-slate-50 dark:bg-slate-700/50 transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center">
                                            <FileText className="w-8 h-8 text-brand-600 dark:text-brand-400 mr-3" />
                                            <div>
                            ...
                            <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl transition-colors">
                                <h3 className="font-semibold text-blue-900 dark:text-blue-300 mb-2">Formato Esperado</h3>
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="bg-slate-100 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 text-right font-semibold text-slate-700 dark:text-slate-300">CE</th>
                                                <th className="px-4 py-3 text-right font-semibold text-slate-700 dark:text-slate-300">SC</th>
                                                <th className="px-4 py-3 text-right font-semibold text-slate-700 dark:text-slate-300">SP</th>
                                        ...
                                        <tbody>
                                            {parsedData.slice(0, 10).map((product, index) => (
                                                <tr key={index} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-right text-slate-900 dark:text-white">{product.stock_ce}</td>
                                                    <td className="px-4 py-3 text-right text-slate-900 dark:text-white">{product.stock_sc}</td>
                                                    <td className="px-4 py-3 text-right text-slate-900 dark:text-white">{product.stock_sp}</td>
                                                    <td className="px-4 py-3 text-right font-semibold text-slate-900 dark:text-white">{product.total}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
