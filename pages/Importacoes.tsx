...
function ItemThumb({ imageUrl, name }: { imageUrl?: string; name: string }) {
    ...
    return <PackageSearch className="w-10 h-10 text-slate-300" strokeWidth={1.5} />;
}
                    ...
                    {!selectedGroup ? (
                        <div className="flex items-center justify-between flex-wrap gap-4">
                            <div className="flex items-center gap-4">
                                <div className="w-11 h-11 sm:w-[46px] sm:h-[46px] rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0">
                        ...
                        {selectedGroup.items.map((item) => {
                            ...
                            return (
                                <div key={item.id} className="app-card overflow-hidden flex flex-col">
                                    <div className="h-44 w-full bg-slate-50 flex items-center justify-center relative">
                                        <span className="absolute top-3 left-3 inline-flex items-center gap-1 bg-amber-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full tracking-wide shadow-sm">
                                            <Ship className="w-3 h-3" />
                                            A CHEGAR
                                        </span>
                                        <span className={`absolute top-3 right-3 text-white text-[10.5px] font-bold px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1 ${late ? 'bg-red-500' : 'bg-slate-900/80'}`}>
                                    ...
                                    <div className="p-4 flex flex-col flex-1">
                                        ...
                                        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-amber-300/60 dark:border-amber-500/30 mb-2">
                                            <div className="flex items-baseline gap-1.5">
                                                <span className="text-2xl font-black text-amber-600 dark:text-amber-400 leading-none tabular-nums">
                                                    {item.quantity}
                                                </span>
                                                <span className="text-[10.5px] font-bold text-amber-700/70 dark:text-amber-400/70 uppercase tracking-wide">
                                                    un a caminho
                                                </span>
                                            </div>
                                            <Boxes className="w-4 h-4 text-amber-400 dark:text-amber-500/60 shrink-0" />
                                        </div>

                                        <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-2.5">
                                            {item.expectedDate
                                                ? `Previsão de chegada: ${new Date(item.expectedDate).toLocaleDateString('pt-BR')}`
                                                : 'Previsão de chegada a definir'}
                                        </p>
                                        ...
                                        <div className="mt-auto pt-1">
                                            <span className="inline-flex items-center gap-1.5 text-[10.5px] font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                                <Clock className="w-2.5 h-2.5" />
                                                Aguardando chegada
                                            </span>
                                        </div>
